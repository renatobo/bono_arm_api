...
- **Correction (2026-08-06):** the packaged folder changed from `bono_arm_api` to `bono-arm-api`, and this note originally claimed Git Updater would rename the extracted directory so existing installs updated in place. That is wrong. Git Updater installs 2.0.2 as a *second* plugin and leaves the original in place and still active. Deactivate the old `bono_arm_api` copy, keep the new `bono-arm-api` copy active, then delete the old directory over SFTP rather than through the wp-admin Delete link: the 2.0.2 uninstall routine would otherwise delete the settings and capabilities that both copies share. 2.0.3 adds a guard that makes the wp-admin route safe.
